...
async function commonRoutes(request, pathname) {
  ...
  if(pathname==='/api/common/phases/categories'&&request.method==='GET')return json(['问卷及测评反馈','个人全勤','考核优秀奖励','学习输出','课堂互动','特殊调整'])
...
async function adminExtendedRoutes(request, pathname, url) {
  ...
  if(pathname==='/api/admin/rule-text'&&request.method==='PUT'){
    const i=await body(request),latest=await one('SELECT id FROM rule_texts ORDER BY id DESC LIMIT 1')
    if(latest)return json(await one('UPDATE rule_texts SET title=$1,content=$2,updated_at=NOW() WHERE id=$3 RETURNING *',[i.title||'积分规则',i.content,latest.id]))
    return json(await one('INSERT INTO rule_texts(title,content) VALUES($1,$2) RETURNING *',[i.title||'积分规则',i.content]),201)
  }
