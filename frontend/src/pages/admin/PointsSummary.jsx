...
export default function AdminPointsSummary({ type = 'personal' }) {
  ...
  return <AppLayout>
    <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
      <div><h1 className="text-2xl font-bold text-slate-900">{isTeam ? '团队积分管理' : '个人积分管理'}</h1><p className="mt-1 text-sm text-slate-500">{selectedYear?.name || '未选择年度'} · {selectedProject?.name || '请先选择项目'}</p></div>
