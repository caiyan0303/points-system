  ...
  return <AppLayout>
    ...
    {!projectId ? <div className="rounded-[28px] border border-dashed border-indigo-200 bg-white/70 px-8 py-20 text-center shadow-sm"><Layers className="mx-auto h-12 w-12 text-indigo-300" /><h2 className="mt-4 text-xl font-black text-slate-800">请先选择年度和培训项目</h2><p className="mt-2 text-sm text-slate-500">选择后才能查看排行榜、积分汇总并进入个人或团队积分管理。</p></div> : loading && !dashboard ? <div className="flex h-80 items-center justify-center"><div className="h-10 w-10 animate-spin rounded-full border-2 border-indigo-200 border-b-indigo-600" /></div> : error ? <div className="rounded-2xl bg-rose-50 p-8 text-center text-sm font-bold text-rose-600">{error}</div> : dashboard && <>
      ...
      <section className="mb-5 grid gap-5 xl:grid-cols-[1.65fr_.75fr]">
        ...
        <div className="rounded-[28px] border border-indigo-100 bg-white p-6 shadow-xl shadow-indigo-100/30"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.16em] text-indigo-500">Team Champion</p><h2 className="mt-1 text-xl font-black text-slate-900">项目团队第一名</h2></div><Trophy className="h-9 w-9 text-indigo-300" /></div>{teamChampion ? <div className="mt-7 rounded-2xl bg-slate-50 p-5"><div className="flex items-end justify-between gap-3"><div><span className="rounded-full bg-indigo-100 px-3 py-1 text-[10px] font-black text-indigo-600">团队第一名</span><h3 className="mt-3 text-2xl font-black text-slate-900">{teamChampion.name || teamChampion.group_name}</h3></div><p className="text-2xl font-black text-indigo-600">{number(teamChampion.final_score ?? teamChampion.total_points)}<span className="ml-1 text-xs">分</span></p></div><p className="mt-3 text-xs text-slate-400">成员个人积分 {number(teamChampion.personal_points)} ＋ 团队积分 {number(teamChampion.team_points)}</p></div> : <p className="py-16 text-center text-sm text-slate-400">当前项目暂无小组数据</p>}<button onClick={() => navigate('/admin/team-points')} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-indigo-100 bg-indigo-50 px-4 py-3 text-sm font-black text-indigo-700 hover:bg-indigo-100">查看团队积分 <ArrowRight className="h-4 w-4" /></button></div>
      </section>
