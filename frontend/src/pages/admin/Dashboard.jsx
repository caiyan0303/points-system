...
export default function AdminDashboard() {
  ...
  const activities = useMemo(() => [
    ...
    ...(dashboard?.teamRecords || []).map((item) => ({ ...item, type: '团队', owner: item.group_name, label: item.category || '团队积分' })),
  ...
  const quickActions = [
    ...
    { label: '团队积分录入', icon: UsersRound, path: '/admin/team-points', tone: 'from-violet-500 to-fuchsia-500' },
    { label: '团队积分流水', icon: FileSpreadsheet, path: '/admin/team-points/records', tone: 'from-fuchsia-500 to-pink-500' },
  ...
  return <AppLayout>
    <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
      <div><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[.2em] text-indigo-500"><Sparkles className="h-4 w-4" />Project Points Board</p><h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900">项目积分看板</h1><p className="mt-2 text-sm text-slate-500">查看各阶段个人冠军、团队冠军和项目积分运营概况。</p></div>
    ...
    {!projectId ? <div className="rounded-[28px] border border-dashed border-indigo-200 bg-white/70 px-8 py-20 text-center shadow-sm"><Layers className="mx-auto h-12 w-12 text-indigo-300" /><h2 className="mt-4 text-xl font-black text-slate-800">请先选择年度和培训项目</h2><p className="mt-2 text-sm text-slate-500">选择后才能查看排行榜、积分汇总并进入个人或团队积分管理。</p></div> : loading && !dashboard ? <div className="flex h-80 items-center justify-center"><div className="h-10 w-10 animate-spin rounded-full border-2 border-indigo-200 border-b-indigo-600" /></div> : error ? <div className="rounded-2xl bg-rose-50 p-8 text-center text-sm font-bold text-rose-600">{error}</div> : dashboard && <>
      <section className="mb-5 overflow-hidden rounded-[28px] border border-indigo-100 bg-gradient-to-r from-white via-indigo-50/70 to-violet-50/80 p-6 shadow-xl shadow-indigo-100/30">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between"><div className="shrink-0"><p className="text-xs font-bold text-indigo-500">{selectedYear?.name}</p><h2 className="mt-1 text-2xl font-black text-slate-900">{selectedProject?.name}</h2><p className="mt-2 text-xs text-slate-400">{selectedProject?.start_date ? new Date(selectedProject.start_date).toLocaleDateString('zh-CN') : '未设置开始时间'} — {selectedProject?.end_date ? new Date(selectedProject.end_date).toLocaleDateString('zh-CN') : '未设置结束时间'}</p></div><div className="grid w-full grid-cols-2 gap-3 sm:grid-cols-3 2xl:max-w-4xl 2xl:grid-cols-5">{[
          ['参与人数', summary.students, '人'], ['项目个人积分', summary.personalPoints, '分'], ['可兑换积分', summary.availablePoints, '分'], ['团队任务积分', summary.teamPoints, '分'], ['项目小组', summary.groups, '组'],
        ].map(([label, value, unit]) => <div key={label} className="rounded-2xl border border-white bg-white/75 px-4 py-3 text-center shadow-sm"><p className="text-[11px] font-bold text-slate-400">{label}</p><p className="mt-1 text-xl font-black text-indigo-700">{value.toLocaleString()}<span className="ml-1 text-[10px] text-slate-400">{unit}</span></p></div>)}</div></div>
      </section>
      ...
      <section className="mb-5 grid gap-5 xl:grid-cols-[1.65fr_.75fr]">
        ...
        <div className="relative overflow-hidden rounded-[28px] bg-gradient-to-br from-indigo-700 via-violet-700 to-fuchsia-600 p-6 text-white shadow-xl shadow-violet-300/30"><div className="absolute -right-16 -top-16 h-56 w-56 rounded-full bg-white/15 blur-3xl" /><div className="relative"><p className="text-xs font-black uppercase tracking-[.16em] text-violet-100">Team Champion</p><h2 className="mt-1 text-xl font-black">项目团队第一名</h2>{teamChampion ? <div className="mt-8"><div className="mx-auto flex h-28 w-28 items-center justify-center rounded-[34px] border border-white/30 bg-white/15 shadow-2xl backdrop-blur"><Trophy className="h-16 w-16 text-violet-100" /></div><div className="mt-6 text-center"><span className="rounded-full bg-white/15 px-3 py-1 text-[10px] font-black">团队冠军</span><h3 className="mt-3 text-3xl font-black">{teamChampion.name || teamChampion.group_name}</h3><p className="mt-2 text-2xl font-black text-violet-100">{number(teamChampion.final_score ?? teamChampion.total_points)} 分</p><p className="mt-2 text-xs text-violet-200">成员个人积分 {number(teamChampion.personal_points)} ＋ 团队积分 {number(teamChampion.team_points)}</p></div></div> : <p className="py-20 text-center text-sm text-violet-200">当前项目暂无小组数据</p>}<button onClick={() => navigate('/admin/team-points')} className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-white/15 px-4 py-3 text-sm font-black backdrop-blur hover:bg-white/20">查看团队积分 <ArrowRight className="h-4 w-4" /></button></div></div>
      </section>

      <section className="grid gap-5 xl:grid-cols-[1.6fr_.8fr]">
        <div className="rounded-[28px] border border-indigo-100 bg-white p-6 shadow-xl shadow-indigo-100/30"><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.16em] text-cyan-600">Latest Activities</p><h2 className="mt-1 text-xl font-black text-slate-900">积分动态（最近10条）</h2></div></div>{activities.length ? <div className="grid gap-x-6 md:grid-cols-2">{activities.map((item) => <div key={`${item.type}-${item.id}`} className="flex items-center gap-3 border-b border-slate-50 py-3"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${item.type === '团队' ? 'bg-violet-100 text-violet-600' : 'bg-indigo-100 text-indigo-600'}`}>{item.type === '团队' ? <UsersRound className="h-4 w-4" /> : <Award className="h-4 w-4" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-slate-800">{item.owner || '未知对象'} <span className="ml-1 text-xs font-bold text-slate-400">获得</span> <span className={item.type === '团队' ? 'text-violet-600' : 'text-indigo-600'}>{number(item.points) > 0 ? '+' : ''}{number(item.points)} 分</span></p><p className="mt-1 truncate text-[11px] text-slate-400">{item.label} · {item.phase_name || '未关联阶段'}</p></div><span className="shrink-0 text-[10px] text-slate-400">{dateText(item.created_at || item.obtained_date)}</span></div>)}</div> : <p className="py-12 text-center text-sm text-slate-400">当前项目暂无积分动态</p>}</div>
