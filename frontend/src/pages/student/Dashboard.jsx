  ...
  return <AppLayout>
    ...
    <section className="mb-5 grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
      <button onClick={() => navigate('/student/points')} className="group relative min-h-[210px] overflow-hidden rounded-[26px] bg-gradient-to-br from-indigo-600 via-violet-600 to-blue-500 p-6 text-left text-white shadow-2xl shadow-indigo-400/25 soft-grid">
        ...
        <div className="relative z-10 flex h-full max-w-[70%] flex-col justify-between"><div><p className="text-base font-semibold text-indigo-100">个人总积分（当前项目）</p><div className="mt-1 flex items-end gap-2"><strong className="text-5xl font-black tracking-tight sm:text-6xl">{data.period_points || 0}</strong><span className="pb-1.5 text-lg font-bold text-indigo-100">分</span></div></div><div className="mt-5 grid grid-cols-2 gap-6"><div><p className="text-sm text-indigo-200">当前阶段排名</p><strong className="mt-1 block text-xl">{data.current_phase_rank ? `第 ${data.current_phase_rank} 名` : '-'}</strong></div><div><p className="text-sm text-indigo-200">小组排名</p><strong className="mt-1 block text-xl">{teamRank ? `第 ${teamRank} 名` : '-'}</strong></div></div></div>
      </button>
    ...
    <section className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
      ...
      <div className="glass-panel rounded-[30px] border p-6 shadow-xl shadow-indigo-100/40"><div className="mb-4 flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-cyan-600">Points Ledger</p><h2 className="mt-1 text-xl font-black text-slate-900">积分流水动态</h2></div><button onClick={() => navigate(projectViewPath(activityTab === 'personal' ? 'personal' : 'team'))} className="inline-flex items-center gap-1 text-xs font-bold text-indigo-600">查看全部 <ArrowRight className="h-3.5 w-3.5" /></button></div><div className="relative mb-4 grid grid-cols-2 rounded-2xl bg-slate-100/80 p-1"><span className={`absolute bottom-1 top-1 w-[calc(50%-4px)] rounded-xl bg-white shadow-sm transition-transform ${activityTab === 'team' ? 'translate-x-[100%]' : 'translate-x-0'}`} /><button onClick={() => setActivityTab('personal')} className={`relative z-10 rounded-xl px-2 py-2 text-xs font-black ${activityTab === 'personal' ? 'text-indigo-700' : 'text-slate-400'}`}>个人积分流水</button><button onClick={() => setActivityTab('team')} className={`relative z-10 rounded-xl px-2 py-2 text-xs font-black ${activityTab === 'team' ? 'text-violet-700' : 'text-slate-400'}`}>小组积分流水</button></div><div className="space-y-2">{activityRows.length ? activityRows.slice(0, 5).map((item) => { const value = Number(item.points || 0); const dateValue = item.created_at || item.obtained_date; return <div key={`${activityTab}-${item.id}`} className="group flex items-start gap-3 rounded-2xl p-3 transition hover:bg-white/70"><div className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${activityTab === 'team' ? 'bg-violet-100 text-violet-600' : value >= 0 ? 'bg-indigo-100 text-indigo-600' : 'bg-rose-100 text-rose-600'}`}>{activityTab === 'team' ? <Users className="h-4 w-4" /> : <TrendingUp className="h-4 w-4" />}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className="truncate text-sm font-black text-slate-800">{item.category || item.item_name || item.description || (activityTab === 'team' ? '小组积分' : '个人积分')}</p><span className="shrink-0 text-[10px] text-slate-400">{dateValue ? new Date(dateValue).toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit' }) : '-'}</span></div><p className="mt-1 text-xs text-slate-400">{item.phase_name || (activityTab === 'team' ? '小组任务积分' : '个人积分已更新')} · <strong className={activityTab === 'team' ? 'text-violet-600' : value >= 0 ? 'text-indigo-600' : 'text-rose-600'}>{value > 0 ? '+' : ''}{value} 分</strong></p></div></div> }) : <p className="py-10 text-center text-sm text-slate-400">暂无{activityTab === 'personal' ? '个人' : '小组'}积分流水</p>}</div></div>
    </section>
  </AppLayout>
}
