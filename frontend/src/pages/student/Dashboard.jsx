      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl border border-blue-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900 flex items-center gap-2"><Award className="w-4 h-4 text-blue-600" /> 个人排名</h3>
            <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-600">{data.year_name || '当前年度'}</span>
          </div>
          <p className="text-3xl font-bold text-gray-900">{data.period_rank ? `第 ${data.period_rank} 名` : '-'}</p>
          <div className="grid grid-cols-2 gap-3 mt-4">
            <div className="rounded-lg bg-indigo-50 p-3">
              <p className="text-xs text-gray-500 flex items-center gap-1"><TrendingUp className="w-3.5 h-3.5" /> 个人累计积分</p>
              <p className="text-xl font-bold text-indigo-700 mt-1">{data.personal_cumulative_points || 0}</p>
            </div>
            <div className="rounded-lg bg-purple-50 p-3">
              <p className="text-xs text-gray-500 flex items-center gap-1"><ShoppingBag className="w-3.5 h-3.5" /> 可兑换积分</p>
              <p className="text-xl font-bold text-purple-700 mt-1">{data.available_points || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-indigo-200 p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              {data.current_phase && <span className="text-xs px-2 py-0.5 rounded-full bg-green-50 text-green-600">进行中</span>}
            </div>
            <p className="text-lg font-semibold text-gray-900">{data.current_phase ? String(data.current_phase) : '暂无进行中的阶段'}</p>
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className="rounded-lg bg-green-50 p-3">
                <p className="text-xs text-gray-500 flex items-center gap-1"><Layers className="w-3.5 h-3.5" /> 当前阶段个人积分</p>
                <p className="text-xl font-bold text-green-700 mt-1">{data.current_phase_points || 0}</p>
              </div>
              <div className="rounded-lg bg-gray-50 p-3">
                <p className="text-xs text-gray-500">阶段个人排名</p>
                <p className="text-xl font-bold text-gray-800 mt-1">{data.current_phase_rank ? `第 ${data.current_phase_rank} 名` : '-'}</p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl border border-orange-200 p-5">
            <h3 className="font-semibold text-gray-900 flex items-center gap-2 mb-3"><Users className="w-4 h-4 text-orange-600" /> 小组排名</h3>
            <p className="text-3xl font-bold text-gray-900">{data.group_rank ? `第 ${data.group_rank} 名` : '-'}</p>
            <p className="text-sm text-gray-500 mt-1">小组: {data.group_name || '-'}</p>
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className="rounded-lg bg-yellow-50 p-3">
                <p className="text-xs text-gray-500">团队最终得分</p>
                <p className="text-xl font-bold text-yellow-700 mt-1">{data.team_final_score || 0}</p>
              </div>
              <div className="rounded-lg bg-orange-50 p-3">
                <p className="text-xs text-gray-500">团队任务积分</p>
                <p className="text-xl font-bold text-orange-700 mt-1">{data.team_points || 0}</p>
              </div>
            </div>
          </div>
      </div>
