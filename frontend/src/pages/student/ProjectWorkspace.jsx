...
export default function StudentProjectWorkspace({ view = 'overview' }) {
  ...
  return <AppLayout>
    ...
    <section className="mb-6 flex flex-col justify-between gap-5 lg:flex-row lg:items-start"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-indigo-500">{view === 'personal' ? 'Personal Points' : view === 'team' ? 'Group Points' : 'Project Overview'}</p><h1 className="mt-2 text-3xl font-black text-slate-900">{view === 'personal' ? '个人积分' : view === 'team' ? '小组积分' : title}</h1>{view !== 'overview' && <p className="mt-1 text-sm font-semibold text-slate-500">{title}</p>}<p className="mt-3 flex flex-wrap items-center gap-4 text-xs text-slate-400"><span className="flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5" />{dateText(project?.start_date)} - {dateText(project?.end_date)}</span><span className="flex items-center gap-1.5"><Users className="h-3.5 w-3.5" />我的小组：{stats?.group_name || group.name || '暂未分组'}</span></p></div>
      <div className="glass-panel flex items-center gap-7 rounded-3xl border px-6 py-4"><div><p className="text-[10px] text-slate-400">{view === 'team' ? '小组积分' : '当前积分'}</p><p className="mt-1 text-3xl font-black text-indigo-600">{view === 'team' ? teamFinalScore : stats?.period_points || 0}<span className="ml-1 text-xs">分</span></p></div><div className="h-10 w-px bg-slate-100" /><div><p className="text-[10px] text-slate-400">{view === 'team' ? '小组排名' : '当前排名'}</p><p className="mt-1 text-xl font-black text-slate-800">{(view === 'team' ? teamRank : stats?.period_rank) ? `第 ${view === 'team' ? teamRank : stats.period_rank} 名` : '暂无'}</p></div><Trophy className="h-11 w-11 text-amber-400" /></div>
    </section>
