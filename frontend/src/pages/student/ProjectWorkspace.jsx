...
import { ArrowLeft, ArrowRight, CalendarDays, Crown, Gift, Medal, Trophy, UserRound, Users } from 'lucide-react'
...
export default function StudentProjectWorkspace({ view = 'overview' }) {
  ...
  return <AppLayout>
    ...
    {view === 'personal' && <section className="grid gap-5 lg:grid-cols-3">{phases.map((phase, index) => {
      ...
      return <div key={phase.phase_id} className={`rounded-[26px] border bg-gradient-to-br p-6 shadow-lg shadow-slate-200/30 ${phaseTone[index % phaseTone.length]}`}><div className="mb-5 flex items-start justify-between"><div><p className="text-xs font-bold text-slate-500">第 {index + 1} 阶段</p><h2 className="mt-1 text-lg font-black text-slate-900">{phase.phase_name}</h2></div>{Number(phase.rank) === 1 && <span className="flex items-center gap-1 rounded-full bg-amber-100 px-2 py-1 text-[10px] font-black text-amber-700"><Crown className="h-3 w-3" />阶段冠军</span>}</div><div className="mb-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-white/65 p-3"><p className="text-[10px] text-slate-400">我的积分</p><p className="mt-1 text-2xl font-black text-indigo-600">{phase.points || 0}<span className="ml-1 text-xs">分</span></p></div><div className="rounded-2xl bg-white/65 p-3"><p className="text-[10px] text-slate-400">阶段排名</p><p className="mt-1 text-xl font-black text-slate-800">{phase.rank ? `第 ${phase.rank} 名` : '暂无'}</p></div></div><p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">阶段积分 TOP3</p><div className="space-y-2">{rankings.length ? rankings.map((item, rankIndex) => <div key={item.student_id} className="flex items-center gap-2 rounded-xl bg-white/55 px-3 py-2"><span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-black ${rankIndex === 0 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>{rankIndex + 1}</span><span className="min-w-0 flex-1 truncate text-xs font-bold text-slate-700">{item.student_name}</span><strong className="text-xs text-indigo-600">{item.total_points || 0} 分</strong></div>) : <p className="py-5 text-center text-xs text-slate-400">暂无排行数据</p>}</div></div>
    })}</section>}

    {view === 'team' && <section className="grid gap-5 lg:grid-cols-[1.35fr_.65fr]"><div className="glass-panel rounded-[28px] border p-6"><div className="mb-6 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-violet-500">Team Ranking</p><h2 className="mt-1 text-xl font-black text-slate-900">团队积分排行榜</h2></div><Medal className="h-7 w-7 text-amber-400" /></div><div className="space-y-4">{groups.map((item, index) => { const score = Number(item.final_score || item.total_points || 0); return <div key={item.id || item.group_id} className={`rounded-2xl p-4 ${item.is_my_group ? 'bg-indigo-50 ring-1 ring-indigo-200' : 'bg-white/50'}`}><div className="mb-2 flex items-center gap-3"><span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-black ${index === 0 ? 'bg-amber-100 text-amber-700' : index === 1 ? 'bg-slate-200 text-slate-600' : index === 2 ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-500'}`}>{index + 1}</span><span className="flex-1 text-sm font-black text-slate-800">{item.name || item.group_name}{item.is_my_group && <span className="ml-2 text-[10px] text-indigo-500">我的小组</span>}</span><strong className="text-sm text-violet-600">{score} 分</strong></div><div className="ml-10 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-violet-500" style={{ width: `${Math.max(4, score / maxTeamScore * 100)}%` }} /></div></div> })}{!groups.length && <p className="py-12 text-center text-sm text-slate-400">暂无团队排名</p>}</div></div><div className="relative overflow-hidden rounded-[28px] bg-gradient-to-br from-indigo-100 via-violet-100 to-fuchsia-100 p-7 ring-1 ring-violet-200"><Gift className="h-12 w-12 text-violet-600" /><h2 className="mt-6 text-xl font-black text-violet-800">恭喜第 1 名团队！</h2><p className="mt-3 text-sm leading-6 text-violet-600">冠军团队将获得团队奖励，继续保持协作与领先表现。</p><div className="mt-8 rounded-2xl bg-white/60 p-4"><p className="text-xs text-violet-500">我的团队</p><p className="mt-1 text-lg font-black text-violet-800">{group.name || stats?.group_name || '暂未分组'}</p><p className="mt-3 text-3xl font-black text-violet-600">{teamFinalScore}<span className="ml-1 text-xs">分</span></p></div></div></section>}
  </AppLayout>
}
