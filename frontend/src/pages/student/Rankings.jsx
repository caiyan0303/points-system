...
export default function StudentRankings() {
  ...
  return <AppLayout>
    <div className="mb-7"><p className="text-xs font-bold uppercase tracking-[.22em] text-indigo-500">Project Leaderboard</p><h1 className="mt-2 text-3xl font-black text-slate-900">项目排行榜</h1><p className="mt-2 text-sm text-slate-500">见证每个阶段的个人冠军与团队荣誉</p></div>
    <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
      <div className="glass-chip inline-flex rounded-2xl p-1"><button onClick={() => setSearchParams({})} className={`rounded-xl px-5 py-2.5 text-sm font-bold ${tab === 'personal' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}>阶段个人榜</button><button onClick={() => setSearchParams({ tab: 'team' })} className={`rounded-xl px-5 py-2.5 text-sm font-bold ${tab === 'team' ? 'bg-white text-violet-700 shadow-sm' : 'text-slate-500'}`}>团队榜</button></div>
