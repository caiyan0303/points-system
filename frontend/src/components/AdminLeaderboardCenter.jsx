...
const rankTone = (index) => {
  if (index === 0) return 'border-indigo-200 bg-gradient-to-br from-indigo-100 via-violet-100 to-cyan-100 text-slate-900 shadow-indigo-200/60'
...
function RankingCard({ row, index, type }) {
  ...
  return <div className={`relative overflow-hidden rounded-[24px] border p-5 shadow-lg ${rankTone(index)} ${primary ? 'border-transparent' : ''}`}>
    ...
    <div className="relative flex items-start justify-between gap-3">
      <div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${primary ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-indigo-50 text-indigo-600'}`}><Icon className="h-5 w-5" /></div>
      <span className={`rounded-full px-3 py-1 text-[11px] font-black ${primary ? 'bg-white/75 text-indigo-700' : 'bg-slate-100 text-slate-500'}`}>NO.{index + 1}</span>
    </div>
    <div className="relative mt-7">
      <p className="truncate text-lg font-black text-slate-900">{displayName(row, type)}</p>
      <p className="mt-1 truncate text-xs text-slate-500">{type === 'personal' ? (row.group_name || '暂未分组') : `${number(row.member_count)} 名成员`}</p>
      <p className="mt-5 text-3xl font-black text-indigo-600">{displayPoints(row, type).toLocaleString()}<span className="ml-1 text-xs font-bold">分</span></p>
      {type === 'team' && <p className="mt-2 text-[11px] text-slate-500">成员积分 {number(row.personal_points)} · 小组积分 {number(row.team_points)}</p>}
    </div>
  </div>
}
...
export default function AdminLeaderboardCenter({ phases = [], phaseRankings = {}, personalRows = [], groupRows = [] }) {
  ...
  return <section className="mb-6 overflow-hidden rounded-[30px] border border-indigo-100 bg-white shadow-xl shadow-indigo-100/40">
    <div className="border-b border-indigo-100 bg-gradient-to-r from-indigo-50 via-violet-100 to-cyan-100 px-6 py-6 text-slate-900">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex items-center gap-4"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-500 text-white shadow-lg shadow-indigo-200"><Trophy className="h-6 w-6" /></div><div><p className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.22em] text-violet-600"><Sparkles className="h-3.5 w-3.5" />Ranking Center</p><h2 className="mt-1 text-2xl font-black">项目积分排行榜</h2><p className="mt-1 text-xs text-slate-500">项目总榜与各阶段排名统一查看</p></div></div>
        <div className="flex flex-wrap gap-2">
          <div className="flex rounded-2xl bg-white/65 p-1 ring-1 ring-indigo-100">{[['project', '项目总榜'], ['phase', '阶段榜']].map(([value, label]) => <button key={value} onClick={() => setScope(value)} className={`rounded-xl px-4 py-2 text-xs font-black transition ${scope === value ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'text-slate-500 hover:bg-indigo-50'}`}>{label}</button>)}</div>
          <div className="flex rounded-2xl bg-white/65 p-1 ring-1 ring-indigo-100">{[['personal', '个人排名'], ['team', '小组排名']].map(([value, label]) => <button key={value} onClick={() => setType(value)} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-black transition ${type === value ? 'bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white shadow-lg shadow-violet-200' : 'text-slate-500 hover:bg-violet-50'}`}>{value === 'personal' ? <UserRound className="h-3.5 w-3.5" /> : <UsersRound className="h-3.5 w-3.5" />}{label}</button>)}</div>
        </div>
      </div>
    </div>
