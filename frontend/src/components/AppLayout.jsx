...
function SidebarGroup({ group, location, collapsed }) {
  ...
  if (collapsed) return <div className="space-y-1 border-b border-indigo-100 pb-2 last:border-0">
    ...
    {group.items.map((item) => {
      ...
      return <Link key={item.path} to={item.path} title={item.label} aria-label={item.label} className={`mx-auto flex h-10 w-11 items-center justify-center rounded-xl transition ${active ? 'bg-gradient-to-br from-indigo-500 to-violet-600 text-white shadow-lg shadow-indigo-200' : 'text-slate-400 hover:bg-indigo-50 hover:text-indigo-700'}`}><item.icon className="h-4 w-4" /></Link>
    })}
  </div>
  return <div>
    <button onClick={() => setOpen(!open)} className="flex w-full items-center gap-2 px-3 py-2 text-xs font-black tracking-wider text-slate-400 hover:text-indigo-700">
    ...
    {open && <div className="space-y-0.5 mt-0.5 mb-2">{group.items.map((item) => {
      const active = isItemActive(item)
      return <Link key={item.path} to={item.path} className={`flex items-center gap-3 rounded-xl py-2.5 pl-7 pr-3 text-sm font-semibold transition ${active ? 'bg-gradient-to-r from-indigo-500 to-violet-600 text-white shadow-lg shadow-indigo-200' : 'text-slate-500 hover:bg-indigo-50 hover:text-indigo-700'}`}>
...
function AdminLayout({ children, user, logout }) {
  ...
  return <div className="min-h-screen bg-[#f4f6ff] flex text-slate-900">
    <aside className={`${collapsed ? 'w-20' : 'w-64'} fixed inset-y-0 left-0 z-30 flex flex-col border-r border-indigo-100 bg-white text-slate-800 shadow-2xl shadow-indigo-100/70 transition-[width] duration-300`}>
      <div className={`${collapsed ? 'px-3 py-4' : 'p-5'} border-b border-indigo-100 transition-all`}><div className="flex items-center justify-between gap-2"><Link to="/admin/dashboard" title="优才项目积分管理系统" className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3'}`}><div className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-400 via-blue-500 to-violet-500 shadow-lg shadow-indigo-200"><Trophy className="h-5 w-5 text-white" /><span className="absolute -right-1 -top-1 h-3 w-3 rounded-full border-2 border-white bg-amber-300" /></div>{!collapsed && <div><h1 className="font-black text-sm tracking-wide text-slate-800">优才项目积分管理系统</h1><p className="mt-0.5 text-[10px] uppercase tracking-[.2em] text-indigo-500">管理端</p></div>}</Link>{!collapsed && <button onClick={toggleSidebar} title="收起侧边栏" aria-label="收起侧边栏" className="rounded-xl p-2 text-slate-400 transition hover:bg-indigo-50 hover:text-indigo-700"><PanelLeftClose className="h-4 w-4" /></button>}</div>{collapsed && <button onClick={toggleSidebar} title="展开侧边栏" aria-label="展开侧边栏" className="mx-auto mt-3 flex h-8 w-11 items-center justify-center rounded-xl bg-indigo-50 text-indigo-500 transition hover:bg-indigo-100 hover:text-indigo-700"><PanelLeftOpen className="h-4 w-4" /></button>}</div>
      ...
      <div className={`${collapsed ? 'mx-2 grid grid-cols-1 gap-1' : 'mx-3 grid grid-cols-2 gap-2'} mb-2 border-t border-indigo-100 pt-3`}><Link to="/admin/yearly" title="年度汇总" className={`flex items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 hover:bg-indigo-100 ${collapsed ? 'h-10' : 'flex-col gap-1 px-2 py-2 text-[10px]'}`}><Calendar className="h-4 w-4" />{!collapsed && '年度汇总'}</Link><Link to="/admin/operation-logs" title="操作记录" className={`flex items-center justify-center rounded-xl bg-violet-50 text-violet-600 hover:bg-violet-100 ${collapsed ? 'h-10' : 'flex-col gap-1 px-2 py-2 text-[10px]'}`}><FileText className="h-4 w-4" />{!collapsed && '操作记录'}</Link></div>
      <div className="border-t border-indigo-100 p-3"><div className={`flex items-center py-2 ${collapsed ? 'justify-center' : 'gap-2.5 px-2'}`} title={collapsed ? user?.real_name : undefined}><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-50"><User className="h-4 w-4 text-indigo-500" /></div>{!collapsed && <div className="min-w-0 flex-1"><p className="truncate text-xs font-bold text-slate-700">{user?.real_name}</p><p className="text-[10px] text-slate-400">系统管理员</p></div>}</div><button onClick={handleLogout} title="退出登录" className={`mt-1 flex items-center rounded-xl text-xs text-slate-400 hover:bg-rose-50 hover:text-rose-500 ${collapsed ? 'mx-auto h-9 w-11 justify-center' : 'w-full gap-2 px-2 py-2'}`}><LogOut className="h-3.5 w-3.5" />{!collapsed && '退出登录'}</button></div>
    </aside>
