...
function SidebarGroup({ group, location, collapsed }) {
  ...
  if (collapsed) return <div className="space-y-1 border-b border-white/5 pb-2 last:border-0">
    ...
    {group.items.map((item) => {
      ...
      return <Link key={item.path} to={item.path} title={item.label} aria-label={item.label} className={`mx-auto flex h-10 w-11 items-center justify-center rounded-xl transition ${active ? 'bg-white text-violet-700 shadow-lg shadow-violet-300/40' : 'text-indigo-100/80 hover:bg-white/15 hover:text-white'}`}><item.icon className="h-4 w-4" /></Link>
    })}
  </div>
  return <div>
    ...
    {open && <div className="space-y-0.5 mt-0.5 mb-2">{group.items.map((item) => {
      const active = isItemActive(item)
      return <Link key={item.path} to={item.path} className={`flex items-center gap-3 pl-7 pr-3 py-2.5 rounded-xl text-sm font-semibold transition ${active ? 'bg-white text-violet-700 shadow-lg shadow-violet-300/40' : 'text-indigo-100/85 hover:bg-white/15 hover:text-white'}`}>
...
function AdminLayout({ children, user, logout }) {
  ...
  return <div className="min-h-screen bg-[#f4f6ff] flex text-slate-900">
    <aside className={`${collapsed ? 'w-20' : 'w-64'} fixed inset-y-0 left-0 z-30 flex flex-col bg-gradient-to-b from-indigo-600 via-violet-600 to-blue-600 text-white shadow-2xl shadow-indigo-300/30 transition-[width] duration-300`}>
      <div className={`${collapsed ? 'px-3 py-4' : 'p-5'} border-b border-white/15 transition-all`}><div className="flex items-center justify-between gap-2"><Link to="/admin/dashboard" title="优才项目积分管理系统" className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3'}`}><div className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-400 via-blue-400 to-violet-400 shadow-lg shadow-violet-300/40"><Trophy className="h-5 w-5 text-white" /><span className="absolute -right-1 -top-1 h-3 w-3 rounded-full border-2 border-violet-500 bg-amber-300" /></div>{!collapsed && <div><h1 className="font-black text-sm tracking-wide">优才项目积分管理系统</h1><p className="mt-0.5 text-[10px] uppercase tracking-[.2em] text-indigo-100">管理端</p></div>}</Link>{!collapsed && <button onClick={toggleSidebar} title="收起侧边栏" aria-label="收起侧边栏" className="rounded-xl p-2 text-indigo-100 transition hover:bg-white/15 hover:text-white"><PanelLeftClose className="h-4 w-4" /></button>}</div>{collapsed && <button onClick={toggleSidebar} title="展开侧边栏" aria-label="展开侧边栏" className="mx-auto mt-3 flex h-8 w-11 items-center justify-center rounded-xl bg-white/10 text-indigo-100 transition hover:bg-white/20 hover:text-white"><PanelLeftOpen className="h-4 w-4" /></button>}</div>
